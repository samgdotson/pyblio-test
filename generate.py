...
# NOTE: config file for pybliometrics is stored in
# $HOME/.config/pybliometrics.cfg

if __name__ == "__main__":
    start_year = 2013
    end_year = 2024

    model_kwds = ['energy modeling','energy system model',
                  'energy systems','energy modelling','MARKAL',
                  'TIMES','ESME']
    uncertainty_kwds = ['uncertainty','stochastic','robust optimization',
                        'sensitivity analysis','monte carlo analysis',
                        'MGA','stochastic programming']

    keyword_combinations = itertools.product(model_kwds, uncertainty_kwds)
    ...
    for year in range(start_year, end_year):
        ...
        # get the results
        x = ScopusSearch(
            f'TITLE-ABS-KEY( "energy modeling" ) AND TITLE-ABS-KEY( MGA )',
            view="STANDARD")
